...
def _maybe_fast_pass(text: str) -> bool:
    """Если в тексте нет ни одного матерного/оскорбительного триггера — пропустить."""
    low = text.lower()
...
# Токенизатор нашей модели ловит обфускации слабо, поэтому для переставленных
# букв (лесенка, «долбоеь», «туопйрылый») добавляем нечёткий матч: ищем слова
# с перестановкой 1-2 соседних букв от базовых оскорбительных слов.
_FUZZY_BASES = (
...
def _has_fuzzy_insult(low: str) -> bool:
    """Ищет обфусцированные формы оскорблений: перестановка ОДНОЙ пары
    соседних букв («туопйрылый» = «тупой», «ухй» = «хуй», «омйо» = «чмо»).
    Обычные подстроки («нахуй», «блядь») НЕ считаются — их оценивает модель."""
