...
# Слова-указатели адресации: обфусцированный МАТ («бялть», «ухй») становится
# оскорблением только если обращён к человеку («ты», «иди») или таких
# искажений несколько. Один паразит без адресата («бялть, ну что за день»)
# — НЕ оскорбление.
_ADDRESS_WORDS = (
    "ты", "тебя", "тебе", "тобой", "твоя", "твоё", "твой", "твои", "твою",
    "вам", "вас", "ваш", "ваша", "ваше", "ваши", "иди", "идите", "пошёл",
    "пошла", "пошли", "валяй", "завались", "заткнись", "отстань", "ублюдок",
    "гнида",
)

# Мат-базы для перестановок («бля»->«бял», «хуй»->«ухй»). Короткие «уёб/уеб»
# исключены: перестановка «убе» ловит невинные «убежал» и т.п.
_MAT_BASES = ("ебан", "ебал", "хуй", "хуя", "хуе", "пизд", "бля", "муд")


def _has_fuzzy_insult(low: str) -> bool:
    """Ищет ИСКАЖЁННЫЕ формы оскорблений:
    A) оскорбление есть в нормализованном тексте, но не как обычное слово
       в исходном («тупооой», лесенка «ЧМОКА», «zалyп»);
    B) перестановка соседних букв в оскорбительных словах
       («туопйрылый»=«тупой», «омйо»=«чмо»);
    C) перестановка в мат-базах («ухй»=«хуй», «бял»=«бля») — только если
       есть адресация к человеку или искажений несколько.
    Обычные формы («нахуй», «блядь», «какой тупой фильм») НЕ сигнал —
    их оценивают модель и маркеры."""
    norm = _normalize(low)
    # A) искажённая форма оскорбления (не мат-паразита)
    if any(b in norm for b in _INSULT_BASES) and not any(b in low for b in _INSULT_BASES):
        return True
    # B) перестановки соседних букв в оскорбительных словах
    for base in _INSULT_BASES:
        if len(base) < 3:
            continue
        for i in range(len(base) - 1):
            ...
            if v in low or v in norm:
                return True
    # C) перестановки в мат-базах
    mat_hits = 0
    for base in _MAT_BASES:
        if len(base) < 3:
            continue
        for i in range(len(base) - 1):
            v = base[:i] + base[i + 1] + base[i] + base[i + 2:]
    ...
    if mat_hits == 0:
        return False
    addressed = any(w in low for w in _ADDRESS_WORDS)
    return mat_hits >= 2 or addressed
