...
async def moderate(rule_desc: str, text: str) -> bool:
    ...
    prompt = (
        ...
        "Учитывай обфусцированный мат: переставленные буквы («ануйх» = «нахуй»), "
        "опечатки, мат по буквам, мат лесенкой — это тоже мат, читай по смыслу. "
...
async def analyze_message(rules_context: str, text: str, extra: str = "") -> dict | None:
    ...
    prompt += (
        ...
        "• Мат — это брань, направленная на человека или используемая как "
        "оскорбление (например «ты мой хуй соси», «пиздец ты тупой», «соси хуй, урод»). "
        "Одиночное упоминание нецензурного слова без адресата — смягчай или пропускай.\n"
        "• Обфусцированный мат — это ТОЖЕ мат, а не бессмыслица: переставленные "
        "буквы («ануйх» = «нахуй», «ебанйо» = «ебаный», «пиздец» = «пиздец», "
        "«шклюхзи» = «шлюхи»), опечатки, мат по буквам или через пробелы/точки "
        "(«н а х у й», «х.у.й»), мат «лесенкой» (каждое слово с новой строки: "
        "«МОЙ», «ХУЙ», «БЕРТ»). Читай по смыслу, а не по буквам. Если такой мат "
        "направлен на человека — нарушение; без адресата — смягчай или пропускай.\n"
        ...
        "мат (переставленные буквы, мат по буквам, мат лесенкой) — это мат, а не бессмыслица.\n"
